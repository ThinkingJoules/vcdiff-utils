...
use crate::{extractor::{get_exact_slice, CopyInst, CopyType, DisCopy, DisInst, ExInstType, ExtractedInst, InstType, Instruction, VcdExtract}, reader::{read_header, read_window_header, WindowSummary}};
...
//     Q(CopyQ),
// }
// impl Instruction for DerefCopy{
//     fn len_in_u(&self)->u32 {
//         match self {
//             DerefCopy::NonQ(a) => a.len_in_u(),
//             DerefCopy::Q(a) => a.len,
...
//         todo!()
//     }
//     fn inst_type(&self)->crate::extractor::InstType {
//         match self {
//             DerefCopy::NonQ(a) => a.inst_type(),
//             DerefCopy::Q(a) => a.inst_type(),
//         }
...
// impl DerefCopy {
//     pub fn take_nonq(self)->DisCopy{
//         match self {
//             DerefCopy::NonQ(a) => a,
//             _ => panic!("Impl Err"),
//         }
//     }
//     pub fn take_q(self)->CopyQ{
//         match self {
//             DerefCopy::Q(a) => a,
//             _ => panic!("Impl Err"),
//         }
//     }
// }
// pub type DerefdInst = ExInstType<DerefCopy>;

// pub type DDInst = DisInst<DerefdInst>;
// impl ExtractedInst {
//     fn to_deref(self)->DerefdInst{
//         match self {
//             ExtractedInst::Add(a) => DerefdInst::Add(a),
//             ExtractedInst::Run(r) => DerefdInst::Run(r),
//             ExtractedInst::Copy(c) => {
//                 if c.in_s(){
//                     DerefdInst::Copy(DerefCopy::NonQ(c))
//                 }else{
//                     panic!("We must resolve Copy before dereferencing")
//                 }
//             },
//         }
//     }
...
//     let mut output = Vec::with_capacity(extracted.len());
//     let slice = extracted.as_slice();
...
//                     let mut seq = CopyQ { inst: slice, len: copy.len_in_o(), skip: 0, trunc: 0 };
//                     output.push(MInst { o_pos_start, inst: DerefdInst::Copy(DerefCopy::Q(seq)) });
//                 }else{
...
//             },
//             a => output.push(DisInst { o_pos_start, inst: a.to_deref() }),
//         }
//     }
//     todo!()
// }
...
//     ResolveQ,
// }
// fn global_deref_cntl(inst: &DDInst)->ResolveDecision{
//     match &inst.inst {
//         DerefdInst::Copy(DerefCopy::NonQ(copy)) if copy.vcd_trgt() => {
//             debug_assert!(copy.in_s(), "We should have resolved all Sourced Copys by now");
//             debug_assert!(!copy.is_implicit_seq(), "We should have sorted out all sequences here.");
//             ResolveDecision::ResolveNonQ
//         },
//         DerefdInst::Copy(DerefCopy::Q(CopyQ { inst, len, skip, trunc })) => {
//             ResolveDecision::ResolveQ
//         },
//         _ => ResolveDecision::NoOp,
//     }
// }
// pub(crate) fn deref_nonq(source:&[DDInst],copy:&DisCopy)->Vec<DDInst>{
...
//     let mut resolved = get_exact_slice(source, o_start, copy.len_in_u()).unwrap();
//     resolved
// }
// fn global_deref_q(cur_output:&[DDInst],seq:CopyQ)->CopyQ{
...
//     for dd in inst {
//         match global_deref_cntl(&dd){
//             ResolveDecision::NoOp => output.push(dd),
//             ResolveDecision::ResolveNonQ => {
//                 let copy = dd.inst.take_copy().unwrap().take_nonq();
...
//             },
//             ResolveDecision::ResolveQ => {
//                 let DisInst { o_pos_start, inst } = dd;
//                 let mut resolved = global_deref_q(cur_output,inst.take_copy().unwrap().take_q());
//                 output.push(DisInst { o_pos_start, inst: DerefdInst::Copy(DerefCopy::Q(resolved)) });
//             },
//         }
//     }
//     CopyQ { inst: output, len, skip, trunc }
// }
// pub fn translate_global_only(locally_resolved:Vec<DDInst>)->Vec<DDInst>{
//     let mut output = Vec::with_capacity(locally_resolved.len());
//     let slice = locally_resolved.as_slice();
//     for dd in locally_resolved {
//         match global_deref_cntl(&dd){
//             ResolveDecision::NoOp => output.push(dd),
//             ResolveDecision::ResolveNonQ => {
//                 let mut resolved = deref_nonq(output.as_slice(), &dd.inst.take_copy().unwrap().take_nonq());
//                 output.append(&mut resolved);
//             },
//             ResolveDecision::ResolveQ => {
//                 let DisInst { o_pos_start, inst } = dd;
//                 let mut resolved = global_deref_q(&output,inst.take_copy().unwrap().take_q());
//                 output.push(DisInst { o_pos_start, inst: DerefdInst::Copy(DerefCopy::Q(resolved)) });
//             },
//         }
//     }
...
//     fn len_in_u(&self)->u32 {
//         self.inst.iter().map(|x|x.len_in_u()).sum::<u32>()
//     }
...
//         InstType::Copy { copy_type: CopyType::CopyQ { len_o:self.len_in_o() }, vcd_trgt: self.inst.iter().any(|f|
//             matches!(f.inst_type(), InstType::Copy { vcd_trgt: true,.. })
//         ) }
//     }
// }
...
// pub struct NonQCopy{pub u_pos:u32,len:u32,pub ssp:u64,vcd_trgt:bool}
...
//             Inst::Copy (NonQCopy{ copy, sss, ssp }) => ProcessInst::Copy { copy: copy.clone(), addr: CopyAddr::InS { sss: *sss, ssp: *ssp },vcd_trgt },
...
//             Self::Sequence(CopyQ { len, skip, trunc, ..}) => {
//                 //cannot be zero len, we have a logical error somewhere
...
//             Self::Sequence(CopyQ { skip, .. }) => {*skip += amt as u32;},
...
//             Self::Sequence(CopyQ {trunc ,..}) => {*trunc += amt as u32;},
//         }
//     }
...
//             ProcessInst::Copy { copy, addr:CopyAddr::InS{ sss, ssp },.. } => Inst::Copy(NonQCopy { copy: copy.clone(), sss:*sss, ssp:*ssp }),
...
//         let same_copy = Inst::Copy (NonQCopy{ copy: COPY { len: 4, u_pos: 0 }, sss: 4, ssp: 1 });
...
//         assert_eq!(msg.inst, Inst::Copy (NonQCopy{ copy: COPY { len: 4, u_pos: 0 }, sss: 4, ssp: 1 }));//ello
...
//         assert_eq!(msg.inst, Inst::Copy (NonQCopy{ copy: COPY { len: 3, u_pos: 0 }, sss: 4, ssp: 1 })); //ell
...
//         assert_eq!(msg.inst, Inst::Sequence(CopyQ { inst: vec![Inst::Add(ADD { len: 1, p_pos: 48 })], len: 2, skip: 0, trunc: 0 }));
...
//         let msg = translator.interrogate(3).unwrap().unwrap();
//         assert_eq!(msg.inst, Inst::Sequence(CopyQ { inst: vec![Inst::Add(ADD { len: 2, p_pos: 13 })], len: 3, skip: 0, trunc: 0 }));
//         let msg = translator.interrogate(6).unwrap().unwrap();
...
//         let msg = translator.interrogate(1).unwrap().unwrap();
//         let same_copy = Inst::Copy (NonQCopy{ copy: COPY { len: 4, u_pos: 0 }, sss: 11, ssp: 1 });
//         assert_eq!(msg.inst, same_copy); //ello

//         let msg = translator.interrogate(5).unwrap().unwrap();
//         assert_eq!(msg.inst, Inst::Copy (NonQCopy{ copy: COPY { len: 1, u_pos: 10 }, sss: 11, ssp: 1 })); // '!'
//         let msg = translator.interrogate(6).unwrap().unwrap();
//         assert_eq!(msg.inst, Inst::Copy (NonQCopy{ copy: COPY { len: 1, u_pos: 4 }, sss: 11, ssp: 1 })); // ' '
//         let msg = translator.interrogate(7).unwrap().unwrap();
//         let big_seq = Inst::Sequence(CopyQ {
//             inst: vec![
//                 Inst::Add(ADD { len: 1, p_pos: 14 }), // H
//                 Inst::Copy(NonQCopy{copy: COPY { len: 4, u_pos: 0 }, sss: 11, ssp: 1 } ),// ello
//                 Inst::Copy(NonQCopy{copy: COPY { len: 1, u_pos: 10 }, sss: 11, ssp: 1 } ),// !
//                 Inst::Copy(NonQCopy{copy: COPY { len: 1, u_pos: 4 }, sss: 11, ssp: 1 } ),// ' '
//             ],
...
//         let big_seq_space = Inst::Sequence(CopyQ {
//             inst: vec![
//                 Inst::Add(ADD { len: 1, p_pos: 14 }), // H
//                 Inst::Copy(NonQCopy{copy: COPY { len: 4, u_pos: 0 }, sss: 11, ssp: 1 } ),// ello
//                 Inst::Copy(NonQCopy{copy: COPY { len: 1, u_pos: 10 }, sss: 11, ssp: 1 } ),// !
...
//         let big_seq_ello = Inst::Sequence(CopyQ {
//             inst: vec![
//                 Inst::Add(ADD { len: 1, p_pos: 14 }), // H
//                 Inst::Copy(NonQCopy{copy: COPY { len: 4, u_pos: 0 }, sss: 11, ssp: 1 } ),// ello
//                 Inst::Copy(NonQCopy{copy: COPY { len: 1, u_pos: 10 }, sss: 11, ssp: 1 } ),// !
...
//         let seq_dot = Inst::Sequence(CopyQ {
//             inst: vec![
